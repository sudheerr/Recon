  ...
  BEGIN
    ...
    --out_error_code := '00';
       ...
       -- AND CORE_PRODUCT.EVENT = SAP_PRODUCT.EVENT
        ...
        SELECT COUNT(*),
        SUM(CASE WHEN CORE_STATUS = 'Success' THEN 1 ELSE 0 END ),
        SUM(CASE WHEN CORE_STATUS != 'Success' THEN 1 ELSE 0 END ),

        SUM(CASE WHEN SAP_EIS_ORDNUM is not null THEN 1 ELSE 0 END ),
        SUM(CASE WHEN STATUS_FOUND='FoundInSAP' THEN 1 ELSE 0 END ),
        SUM(CASE WHEN STATUS_FOUND='MissingInSAP' and SAP_EIS_ORDNUM is not null THEN 1 ELSE 0 END ),
        ...
        INTO v_c_total, v_c_success, v_c_failure,
          ...
          v_s_total, v_s_success, v_s_failure
        FROM (
        SELECT 'FoundInSAP' as STATUS_FOUND, CORE_ORDER.SUB_ORD_REF_NUM, SAP_ORDER.SUB_ORD_REF_NUM AS SAP_EIS_ORDNUM, CORE_ORDER.CORE_REQUEST_DATE, CORE_ORDER.ORDER_PRICE, CORE_ORDER.STATUS_MSG AS CORE_STATUS,    SAP_ORDER.STATUS_CODE, SAP_ORDER.STATUS_MSG  FROM (
        SELECT * FROM (SELECT ID, SUB_ORD_REF_NUM, PURCHASE_ORDER_DATE, ORDER_PRICE,  CORE_REQUEST_DATE, SOURCE_SYSTEM, STATUS_MSG,
        ...
        UNION
        SELECT 'MissingInSAP' as STATUS_FOUND, CORE_SAP_FINAL.SUB_ORD_REF_NUM, EIS_ORDER.SUB_ORD_REF_NUM,            CORE_SAP_FINAL.CORE_REQUEST_DATE, CORE_SAP_FINAL.ORDER_PRICE, CORE_SAP_FINAL.CORE_STATUS AS CORE_STATUS, null, EIS_ORDER.STATUS_MSG AS ST_MSG  FROM (
        SELECT * FROM (
        SELECT  CORE_ORDER.SUB_ORD_REF_NUM, SAP_ORDER.SUB_ORD_REF_NUM as SAP_ORD_NUM, CORE_ORDER.CORE_REQUEST_DATE, CORE_ORDER.ORDER_PRICE, CORE_ORDER.STATUS_MSG AS CORE_STATUS, SAP_ORDER.STATUS_CODE, SAP_ORDER.STATUS_MSG  FROM (
        SELECT * FROM (SELECT ID, SUB_ORD_REF_NUM, PURCHASE_ORDER_DATE, ORDER_PRICE,  CORE_REQUEST_DATE, SOURCE_SYSTEM, STATUS_MSG,
        RANK()  OVER(PARTITION BY SUB_ORD_REF_NUM ORDER BY ID DESC ) AS RANK2
        FROM RECON_CORE_PDMS_ORDER_TXN WHERE CORE_REQUEST_DATE >= startDate and CORE_REQUEST_DATE <= endDate) WHERE  RANK2 =1) CORE_ORDER
        LEFT OUTER JOIN(
        ...
        LEFT OUTER JOIN(SELECT * FROM (SELECT ID, SUB_ORD_REF_NUM, PURCHASE_ORDER_DATE, ORDER_PRICE,  TIBCO_CREATE_DATE, STATUS_MSG,
        ...
        EIS_ERRORS,SAP_TOTAL,SAP_SUCCESS,SAP_ERRORS,CREATED_TS,FLOW_DIRECTION)
        values
        (v_temp,'I0343','PD Microsites','SAP', to_date(to_char(startDate,'YYYY-MM-DD'),'YYYY-MM-DD'), to_date(to_char(endDate,'YYYY-MM-DD'),'YYYY-MM-DD'),'QTCOrderManagement01','UpdateOrderFromPDMicroSites',
          v_c_total, v_c_success, v_c_failure,
          v_e_total, v_e_success, v_e_failure,
          v_s_total, v_s_success, v_s_failure, sysdate, 'I');
        ...
        SUM(CASE WHEN STATUS_FOUND='FoundInSAP' and STATUS_CODE = 53 THEN 1 ELSE 0 END ) SAP_SUCCESS,
        SUM(CASE WHEN STATUS_FOUND='FoundInSAP' and STATUS_CODE <> 53 THEN 1 ELSE 0 END ) SAP_FAILURE
        FROM (
        SELECT 'FoundInSAP' AS STATUS_FOUND, SAP_ORDER.SUB_ORD_REF_NUM, SAP_ORDER.STATUS_CODE, SOURCE_SYSTEM, SAP_ORDER.LINE_ITEM_COUNT, SAP_ORDER.TOTAL_PRICE, SAP_ORDER.SUB_ORD_REF_NUM AS SAP_EIS_ORDER,
        CORE_ORDER.CORE_LOAD_DATE_TIME FROM (
