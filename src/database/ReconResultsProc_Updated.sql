...
CREATE OR REPLACE PACKAGE BODY recon_pkg AS
  ...
  BEGIN
    ...
    INSERT INTO svc_logs( APPLICATION,LOG_LEVEL, LOG_LINE, log_text, create_user, create_date) VALUES ('ServiceCatalog', 'INFO', '1.1', log, user_name, current_date);
        ...
        SUM(CASE WHEN SENT_TO_TIBCO != 'Y' THEN 1 ELSE 0 END ),
        SUM(CASE WHEN EIS_MAT_NUM is not null THEN 1 ELSE 0 END ),
        SUM(CASE WHEN EIS_MAT_NUM is not null and EIS_STATUS = 'SUCCESS'  THEN 1 ELSE 0 END ),
        SUM(CASE WHEN EIS_MAT_NUM is not null and EIS_STATUS <> 'SUCCESS'  THEN 1 ELSE 0 END ),
        ...
        FROM(
        SELECT CORE_MATNUM, CORE_EVENT, CORE_PRODUCT_TYPE,  SENT_TO_TIBCO, EIS_MAT_NUM, EIS_STATUS, SAP_PRODUCT.SAP_MATERIAL_NUMBER AS SAP_MATNUM, SAP_PRODUCT.STATUS AS SAP_STATUS  FROM (
        SELECT CORE_PRODUCT.SAP_MATERIAL_NUMBER  as CORE_MATNUM, CORE_PRODUCT.EVENT AS CORE_EVENT, CORE_PRODUCT.PRODUCT_TYPE AS CORE_PRODUCT_TYPE, CORE_PRODUCT.SENT_TO_TIBCO,  EIS_PRODUCT.SAP_MATERIAL_NUMBER as EIS_MAT_NUM,  EIS_PRODUCT.STATUS AS EIS_STATUS  FROM (
        SELECT * FROM (
        SELECT  RECON_CORE_PRODUCT_TXN.*, RANK() OVER(PARTITION BY SAP_MATERIAL_NUMBER ORDER BY CREATED_TS DESC ) AS RANK1 
        FROM RECON_CORE_PRODUCT_TXN WHERE CORE_CREATE_DATE >= startDate and CORE_CREATE_DATE <= endDate) WHERE RANK1 =1) CORE_PRODUCT
        LEFT OUTER JOIN (
        SELECT * FROM (
        SELECT  RECON_EIS_PRODUCT_TXN.*, RANK() OVER(PARTITION BY SAP_MATERIAL_NUMBER ORDER BY CREATED_TS DESC ) AS RANK2 
        FROM RECON_EIS_PRODUCT_TXN WHERE TIBCO_CREATE_DATE >= startDate and TIBCO_CREATE_DATE <= endDate) WHERE RANK2 =1
        ) EIS_PRODUCT
        ON CORE_PRODUCT.SAP_MATERIAL_NUMBER = EIS_PRODUCT.SAP_MATERIAL_NUMBER) CORE_EIS
        LEFT OUTER JOIN (
        SELECT * FROM (
        SELECT  RECON_SAP_PRODUCT_TXN.*, RANK() OVER(PARTITION BY SAP_MATERIAL_NUMBER ORDER BY CREATED_TS DESC ) AS RANK3 
        FROM RECON_SAP_PRODUCT_TXN WHERE SAP_CREATED_DATE >= startDate and SAP_CREATED_DATE <= endDate) WHERE RANK3 =1) SAP_PRODUCT
        ON CORE_EIS.CORE_MATNUM = SAP_PRODUCT.SAP_MATERIAL_NUMBER); 
        ...
        -- Orders Transactions
        FOR TEMP_RESULTS IN (
        SELECT CORE_SOURCE AS SOURCE_SYSTEM,
        ...
        FROM (
        SELECT CORE_ORDNUM, CORE_SOURCE, CORE_CURRENCY, EIS_ORDNUM, EIS_STATUS, SAP_ORDER.SUB_ORD_REF_NUM  AS SAP_ORDNUM, SAP_ORDER.STATUS AS SAP_STATUS FROM (
        SELECT CORE_ORDER.SUB_ORD_REF_NUM AS CORE_ORDNUM, CORE_ORDER.SOURCE_SYSTEM AS CORE_SOURCE, CORE_ORDER.CURRENCY_CODE AS CORE_CURRENCY,
        EIS_ORDER.SUB_ORD_REF_NUM AS EIS_ORDNUM, EIS_ORDER.STATUS AS EIS_STATUS
        FROM (
        SELECT * FROM(
        SELECT RECON_CORE_ORDER_TXN.*, RANK() OVER (PARTITION BY SUB_ORD_REF_NUM ORDER BY CREATED_TS DESC ) AS RANK1 
        FROM RECON_CORE_ORDER_TXN WHERE CORE_LOAD_DATE_TIME>= startDate and CORE_LOAD_DATE_TIME <= endDate) WHERE RANK1 =1) CORE_ORDER
        LEFT OUTER JOIN(
        SELECT * FROM (
        SELECT RECON_EIS_ORDER_TXN.*, RANK() OVER (PARTITION BY SUB_ORD_REF_NUM ORDER BY CREATED_TS DESC ) AS RANK2 
        FROM RECON_EIS_ORDER_TXN WHERE TIBCO_CREATED_DATE>= startDate and TIBCO_CREATED_DATE <= endDate) WHERE RANK2 =1
        ) EIS_ORDER
        ON CORE_ORDER.SUB_ORD_REF_NUM = EIS_ORDER.SUB_ORD_REF_NUM)CORE_EIS 
        LEFT OUTER JOIN(
         SELECT * FROM (
        SELECT  RECON_SAP_ORDER_TXN.*, RANK() OVER(PARTITION BY SUB_ORD_REF_NUM ORDER BY CREATED_TS DESC ) AS RANK3 
        FROM RECON_SAP_ORDER_TXN WHERE SAP_CREATED_DATE >= startDate and SAP_CREATED_DATE <= endDate) WHERE RANK3 =1) SAP_ORDER
        ON CORE_EIS.CORE_ORDNUM = SAP_ORDER.SUB_ORD_REF_NUM) GROUP BY CORE_SOURCE )
        LOOP
        ...
        FOR TEMP_RESULTS IN (
        SELECT CORE_SOURCE AS SOURCE_SYSTEM, CORE_CURRENCY AS CURRENCY_CODE, COUNT(*) AS SRC_COUNT,
        SUM(TOTAL_PRICE) AS CORE_TOTAL,
        SUM(TOTAL_PRICE) AS CORE_SUCCESS, 0 AS CORE_FAILURE,
        SUM(CASE WHEN EIS_ORDNUM is not null THEN TOTAL_PRICE ELSE 0 END ) TIBCO_TOTAL,
        SUM(CASE WHEN EIS_ORDNUM is not null and EIS_STATUS = 'SUCCESS' THEN TOTAL_PRICE ELSE 0 END ) TIBCO_SUCCESS,
        SUM(CASE WHEN EIS_ORDNUM is not null and EIS_STATUS <> 'SUCCESS' THEN TOTAL_PRICE ELSE 0 END ) TIBCO_FAILURE,
        SUM(CASE WHEN SAP_ORDNUM is not null THEN TOTAL_PRICE ELSE 0 END ) SAP_TOTAL,
        SUM(CASE WHEN SAP_ORDNUM is not null and SAP_STATUS = 'SUCCESS' THEN TOTAL_PRICE ELSE 0 END ) SAP_SUCCESS,
        SUM(CASE WHEN SAP_ORDNUM is not null and SAP_STATUS <> 'SUCCESS' THEN TOTAL_PRICE ELSE 0 END ) SAP_FAILURE
        FROM(
        SELECT CORE_ORDNUM, CORE_SOURCE, CORE_CURRENCY, CORE_TOTALPRICE AS TOTAL_PRICE,  EIS_ORDNUM, EIS_STATUS, SAP_ORDER.SUB_ORD_REF_NUM  AS SAP_ORDNUM, SAP_ORDER.STATUS AS SAP_STATUS FROM (
        SELECT CORE_ORDER.SUB_ORD_REF_NUM AS CORE_ORDNUM, CORE_ORDER.SOURCE_SYSTEM AS CORE_SOURCE, CORE_ORDER.CURRENCY_CODE AS CORE_CURRENCY,
        CORE_ORDER.TOTAL_PRICE AS CORE_TOTALPRICE, EIS_ORDER.SUB_ORD_REF_NUM AS EIS_ORDNUM, EIS_ORDER.STATUS AS EIS_STATUS
        FROM (
        SELECT * FROM(
        SELECT RECON_CORE_ORDER_TXN.*, RANK() OVER (PARTITION BY SUB_ORD_REF_NUM ORDER BY CREATED_TS DESC ) AS RANK1 
        FROM RECON_CORE_ORDER_TXN WHERE CORE_LOAD_DATE_TIME>= startDate and CORE_LOAD_DATE_TIME <= endDate) WHERE RANK1 =1) CORE_ORDER
        ...
        SELECT  RECON_SAP_ORDER_TXN.*, RANK() OVER(PARTITION BY SUB_ORD_REF_NUM ORDER BY CREATED_TS DESC ) AS RANK3 
        FROM RECON_SAP_ORDER_TXN WHERE SAP_CREATED_DATE >= startDate and SAP_CREATED_DATE <= endDate) WHERE RANK3 =1) SAP_ORDER
        ON CORE_EIS.CORE_ORDNUM = SAP_ORDER.SUB_ORD_REF_NUM)  GROUP BY CORE_SOURCE, CORE_CURRENCY )
        LOOP
            v_source := TRIM(TEMP_RESULTS.SOURCE_SYSTEM);
  ...
  AS
    ...
    BEGIN
      ...
      ELSIF errcode = 'EIS_MISS' THEN
        filterCondn:= ' WHERE EIS_ORDNUM is null';
      ELSIF errcode = 'SAP_MISS' THEN
        filterCondn:= ' WHERE EIS_ORDNUM is not null and SAP_ORDNUM is null';
      ELSE
...
CREATE OR REPLACE PACKAGE BODY recon_product_pkg AS
  ...
  AS
    ...
    ELSIF errcode = 'EIS_MISS' THEN
        filterCondn:= ' WHERE SENT_TO_TIBCO =''Y'' and EIS_MATNUM is null';
    ELSIF errcode = 'SAP_MISS' THEN
        filterCondn:= ' WHERE EIS_MATNUM is not null and SAP_MATNUM is null';
    ELSE
        filterCondn:= ' WHERE 1!=1';
    END IF;
