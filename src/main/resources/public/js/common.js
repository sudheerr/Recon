var ReconView = (function() {
    ...
    function getFinancialQuarter(){
        var monthIndex = moment().month();// starts from 0
        if(monthIndex === 0){
            return moment().subtract(2, 'month').startOf('month');
        }else {
            var tempIndex = (3 * (Math.floor((monthIndex - 1) / 3)) + 1);
            return moment().month(tempIndex).startOf('month');
        }
    }
    ...
    return {
        ...
        getFinancialQuarter:getFinancialQuarter,
        isSafari:isSafari
    };
})();
